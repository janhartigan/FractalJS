/** JavaScript FractalJS class
 * 
 * This is a JavaScript class that takes a canvas and turns it into a fractal drawer
 * 
 * @author Jan Hartigan
...
(function() {
	...
	fractal.prototype = {
		...
		buildControlBar: function() {
			var div = document.createElement('div'),
				html = 	'<input type="button" name="zoomin" value="zoom in" />' +
						'<input type="button" name="zoomout" value="zoom out" />',
				self = this;
			
			div.className = 'fractaljs-control-bar';
		...
		/**
		 * Destroys all traces of the fractaljs object
		 */
		destroy: function() {
	...
	function fractalExists(canvas) {
		...
		for (name in fractals) {
			if (canvas == fractals[name].canvas) {
				result = fractals[name];
				break;
			}
		}
	...
	//now create the fractaljs object with will be the front-facing class
	
	/**
	 * the fractaljs object
	 */
	function fractaljs() {}
	...
	fractaljs.prototype = {
		...
		//
		// Methods of the fractaljs object
		//
		
		/**
		 * The creator function which turns any canvas into a fractaljs canvas
		 * 
		 * @param DOMElement	canvas
		 * @param Object		options
		 * 
		 * @return mixed		either the canvas DOMElement or false if failed
		 */
		create: function(canvas, options) {
			var existingFractal = false,
				newFract;
			...
			if (existingFractal) {
				if (options) {
					//if options were supplied, overwrite the default options
					if (options) {
						for (name in existingFractal.options) {
							if (options[name] !== undefined) {
								existingFractal.options[name] = options[name];
							}
						}
					}
				}
			...
			newFract = new fractal(canvas, options);
		...
		/**
		 * this method gets the fractal object associated with the canvas item
		 * 
		 * @param DOMElement	canvas
		 * 
		 * @return mixed		fractal object or false
		 */
		getFractalObject: function(canvas) {
			return fractalExists(canvas);
		},
		
		/**
		 * This method destroys the fractal object related to the supplied canvas
		 * 
		 * @param DOMElement	canvas
		 * 
		 * @return bool			true if the object exists and was destroyed, false if there was no object associated with that canvas
		 */
		destroy: function(canvas) {
			...
			for (name in fractals) {
				if (canvas == fractals[name].canvas) {
					fractals[name].destroy();
					fractals.splice(name, 1);
					return true;
				}
			}
	...
	window.fractaljs = new fractaljs();
})();